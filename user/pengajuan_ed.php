  <?php require_once('head.php');require_once('../koneksi.php'); 
    ...
    $id_p  = $_GET['id_p'];
  $tb_dt = mysqli_query($koneksi,"SELECT * FROM pengajuan INNER JOIN kelompok_tani ON pengajuan.id_k=kelompok_tani.id_k INNER JOIN ketua ON kelompok_tani.id_ke=ketua.id_ke INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w WHERE id_p = '$id_p'");
                      ...
                      <div class="form-group">
                        <label for="">Nama yang Mengajukan</label>
                          <select name="id_ke" class="form-control" required="">
                              <option value="<?=$row['id_ke']?>"><?=$row['username']?></option>
                              <?php 
                                $sss = mysqli_query($koneksi, "SELECT * FROM user");
                                    while($data = mysqli_fetch_array($sss)) {?>
                                    <option name="id_ke" value="<?= $data['id_ke'] ?>"><?= $data['nama'] ?> | <?= $data['level'] ?></option>
                                  <?php }?>
                          </select>
                      </div>
                    </div>
...
if (isset($_POST['edit'])) {
    ...
    if($foto){
    $tambahh = mysqli_query($koneksi,"UPDATE pengajuan SET id_ke='$id_ke',id_k='$id_k',keperluan_p='$keperluan_p',anggaran='$anggaran',file_p='$fl',tgl_p='$now' WHERE `pengajuan`.`id_p`='$id_p'");
    }if($tambahh){
    ...
    $tambah = mysqli_query($koneksi,"UPDATE pengajuan SET id_ke='$id_ke',id_k='$id_k',keperluan_p='$keperluan_p',anggaran='$anggaran',file_p='$namabaru',tgl_p='$now' WHERE `pengajuan`.`id_p`='$id_p'");
if($tambah){
