...
<head>
  ...
  <title>Data Pengajuan Bantuan Kelompok Tani</title>
  ...
  <tbody>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Seluruh Data Kelompok Tani</h5></td>
    </tr>
  </tbody>
</table>
  ...
  <div class="container">
  <h2 class="text-center">DAFTAR PENGAJUAN BANTUAN KELOMPOK TANI </h2>
  ...
  <table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
      ...
      if (isset($_POST['cetak'])) {
        ...
        $result = mysqli_query($koneksi, "SELECT * FROM pengajuan INNER JOIN kelompok_tani ON pengajuan.id_k=kelompok_tani.id_k INNER JOIN ketua ON kelompok_tani.id_ke=ketua.id_ke INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w WHERE pengajuan.tgl_p LIKE '%$bulan%' AND pengajuan.tgl_p LIKE '%$tahun%' ORDER BY nama_k ASC");

      }else{
        $result = mysqli_query($koneksi, "SELECT * FROM pengajuan INNER JOIN kelompok_tani ON pengajuan.id_k=kelompok_tani.id_k INNER JOIN ketua ON kelompok_tani.id_ke=ketua.id_ke INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w");
      }
      if(mysqli_num_rows($result)==0) {
        echo '<script>alert("Data Tidak Ada !"); window.location.href="../admin/cpengajuan.php";</script>';
      }
