...
<head>
  ...
  <tbody>
    <tr>
      <td><h5>Nomor</h5></td>
      <td>:</td>
      <td><h5>4433/ 2342-PEG / UPMDD / 2022</h5></td>
    </tr>
    <tr>
      <td><h5>Sifat</h5></td>
      <td>:</td>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Seluruh Data Posyandu</h5></td>
    </tr>
  </tbody>
</table>
	...
	<div class="container">
	<h2 class="text-center">DAFTAR POSYANDU WILAYAH PROV-KALSEL</h2><br>
	<table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
      ...
      if (isset($_POST['cetak'])) {
        ...
        $result = mysqli_query($koneksi, "SELECT * FROM posyandu INNER JOIN pendamping ON posyandu.id_pd=pendamping.id_pd INNER JOIN daerah ON posyandu.id_d=daerah.id_d WHERE posyandu.tgl LIKE '%$bulan%' AND posyandu.tgl LIKE '%$tahun%' ORDER BY tgl ASC");

      }else{
        $result = mysqli_query($koneksi, "SELECT * FROM posyandu INNER JOIN pendamping ON posyandu.id_pd=pendamping.id_pd INNER JOIN daerah ON posyandu.id_d=daerah.id_d");
      }
...
$no = 1;
			while($data = mysqli_fetch_array($result)) {?>
					<tr>
						...
						<td><?php echo $data['nama_py'] ?></td>
            <td>Kabupaten <?php echo $data['kab'] ?>, Kecamatan <?php echo $data['kec'] ?>, Kelurahan/Desa <?php echo $data['desa'] ?></td>
