...
<head>
  ...
  <tbody>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Seluruh Data Hasil Tani</h5></td>
    </tr>
  </tbody>
</table>
  ...
  <br>
        ...
        $result = mysqli_query($koneksi, "SELECT * FROM hasil INNER JOIN kelompok_tani ON hasil.id_k=kelompok_tani.id_k INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w WHERE hasil.tgl_awal LIKE '%$bulan%' AND hasil.tgl_awal LIKE '%$tahun%' ORDER BY nama_k ASC");

        }else if (isset($_POST['cetak2'])) {
        $nama_k = $_REQUEST['nama_k'];
        $result = mysqli_query($koneksi, "SELECT * FROM hasil INNER JOIN kelompok_tani ON hasil.id_k=kelompok_tani.id_k INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w WHERE nama_k = '$nama_k' ORDER BY nama_k ASC");

        }else{
          $result = mysqli_query($koneksi, "SELECT * FROM hasil INNER JOIN kelompok_tani ON hasil.id_k=kelompok_tani.id_k INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w");
        } 
        if(mysqli_num_rows($result)==0) {
        echo '<script>alert("Data Tidak Ada !"); window.location.href="../admin/chasil.php";</script>';
        
      }?>
  <div class="container">
  <h2 class="text-center">DAFTAR DATA HASIL TANI KT</h2>
