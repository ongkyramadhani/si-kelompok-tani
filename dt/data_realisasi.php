...
<head>
  ...
  <title>Data Realisasi Anggaran Bantuan</title>
  ...
  <tbody>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Realisasi Data Anggaran Bantuan KT</h5></td>
    </tr>
  </tbody>
</table>
  ...
  <br>
        ...
        $result = mysqli_query($koneksi, "SELECT * FROM realisasi INNER JOIN bantuan ON realisasi.id_b=bantuan.id_b INNER JOIN pengajuan ON bantuan.id_p=pengajuan.id_p INNER JOIN kelompok_tani ON pengajuan.id_k=kelompok_tani.id_k INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w WHERE realisasi.tgl_r LIKE '%$bulan%' AND realisasi.tgl_r LIKE '%$tahun%' ORDER BY nama_k ASC");

        }else if (isset($_POST['cetak2'])) {
        $nama_k = $_REQUEST['nama_k'];
        $result = mysqli_query($koneksi, "SELECT * FROM realisasi INNER JOIN bantuan ON realisasi.id_b=bantuan.id_b INNER JOIN pengajuan ON bantuan.id_p=pengajuan.id_p INNER JOIN kelompok_tani ON pengajuan.id_k=kelompok_tani.id_k INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w WHERE nama_k = '$nama_k' ORDER BY nama_k ASC");

        }else{
          $result = mysqli_query($koneksi, "SELECT * FROM realisasi INNER JOIN bantuan ON realisasi.id_b=bantuan.id_b INNER JOIN pengajuan ON bantuan.id_p=pengajuan.id_p INNER JOIN kelompok_tani ON pengajuan.id_k=kelompok_tani.id_k INNER JOIN wilayah_binaan ON kelompok_tani.id_w=wilayah_binaan.id_w");
        } 
        if(mysqli_num_rows($result)==0) {
        echo '<script>alert("Data Tidak Ada !"); window.location.href="../admin/crealisasi.php";</script>';
        
      }

?>
  <div class="container">
  <h2 class="text-center">DATA REALISASI ANGGARAN BANTUAN KT</h2>
  ...
  <table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
      <thead class="">
        <tr class="text-center">
          ...
          <th>Anggaran Setelah Direvisi</th>
          <th>Total Belanja</th>
          <th>Kembalian Belanja</th>
          <th>Belanja NETTO</th>
      ...
      <?php 
        $no = 1;
        while($yaya = mysqli_fetch_array($result)) { ?>
          <tr>
          <td style="vertical-align: middle;"><?=$no++;?></td>
          <td><?= $yaya['kode_b'] ?></td>
          <td><?= tgl_indo($yaya['tgl_r']) ?></td>
          <td><?= $yaya['nama_k'] ?></td>
          <td><?= rupiah($yaya['jumlah_b']) ?></td>
          <td><?= rupiah($yaya['anggaran_r'])?></td>
          <td><?= rupiah($yaya['terpakai_r'])?></td>
          <td><?= rupiah($yaya['kembalian_r'])?></td>
          <td><?= rupiah($yaya['belanja_netto']) ?></td>
          <td><?= round($yaya['realisasi_a'], 2); ?></td>
          <td><?= rupiah($yaya['sisa_a']) ?></td>
          </tr>
        <?php } ?>
      </tbody>
    </thead>
